...
public class RecyclerTouchListener implements RecyclerView.OnItemTouchListener, OnActivityTouchListener {
    ...
    public static long ANIMATION_STANDARD = 300;
    public static long ANIMATION_CLOSE = 150;
...
//        mSwipeListener = listener;
    ...
    public RecyclerTouchListener setOnSwipeListener(OnSwipeListener listener) {
    ...
    public void openSwipeOptions(int position) {
        ...
        closeVisibleBG(mOnSwipeListener, findRootFrameLayout(bgVisibleView));
    ...
    /**
     * Returns the root container (ie. the row holding fg, bg, all the views) for a given view.
     *
     * @param view
     *      view to find parent of
     * @return the root frame layout, or null if we can't find it somehow
     */
    @Nullable
    private ViewGroup findRootFrameLayout(final View view) {
        View currView = view;
        // the root container is a framelayout so
        while ((currView.getParent() instanceof ViewGroup) && !(currView instanceof FrameLayout)) {
            currView = (ViewGroup) currView.getParent();
        }

        return (ViewGroup) currView;
    }
    @Deprecated
    ...
    public void closeVisibleBG(final OnSwipeListener mSwipeCloseListener, final View downView) {
        if (bgVisibleView == null) {
            Log.e(TAG, "No rows found for which background options are visible");
            return;
        }
...
//                Log.i("connie", "closeVisibleBg -- onAnimationStart -- bgVisiblePosition: " + bgVisiblePosition);
                if (mOnSwipeListener != null) {
                    mOnSwipeListener.onForegroundAnimationStart(false, ANIMATION_CLOSE, downView, bgView);
                }
            }

            @Override
            public void onAnimationEnd(Animator animation) {
//                Log.i("connie", "closeVisibleBg -- onAnimationEnd -- bgVisiblePosition: " + bgVisiblePosition);
                if (mSwipeCloseListener != null)
                    mSwipeCloseListener.onSwipeOptionsClosed(fgView, bgView);
    ...
    private void animateFG(final View downView, final Animation animateType, final long duration) {
        ...
        translateAnimator.addListener(new Animator.AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animation) {
//                Log.i("connie", "onAnimationStart: " + animateType);
                final boolean isFgOpening = animateType == RecyclerTouchListener.Animation.OPEN;
                if (mOnSwipeListener != null) {
                    mOnSwipeListener.onForegroundAnimationStart(isFgOpening, duration, downView, bgView);
                }
            }

            @Override
            public void onAnimationEnd(Animator animation) {
//                Log.i("connie", "onAnimationEnd: " + animateType);
                if (mOnSwipeListener != null) {
                    if (animateType == Animation.OPEN)
                        mOnSwipeListener.onSwipeOptionsOpened(fgView, bgView);
                    else if (animateType == Animation.CLOSE)
                        mOnSwipeListener.onSwipeOptionsClosed(fgView, bgView);
                }
    ...
    private void animateFG(final View downView, final Animation animateType, final long duration,
        ...
        translateAnimator.addListener(new Animator.AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animation) {
                final boolean isFgOpening = animateType == Animation.OPEN;
                if (mOnSwipeListener != null) {
                    mOnSwipeListener.onForegroundAnimationStart(isFgOpening, duration, downView, bgView);
                }
            }
...
//                Log.i("connie", "onAnimationEnd: " + animateType);
                if (mSwipeCloseListener != null) {
                    if (animateType == Animation.OPEN)
                        mSwipeCloseListener.onSwipeOptionsOpened(fgView, bgView);
                    else if (animateType == Animation.CLOSE)
                        mSwipeCloseListener.onSwipeOptionsClosed(fgView, bgView);
                }
                translateAnimator.removeAllListeners();
            ...
            @Override
            public void onAnimationCancel(Animator animation) {
//                Log.i("connie", "onAnimationCancel: " + animateType);
            }

            @Override
            public void onAnimationRepeat(Animator animation) {
//                Log.i("connie", "onAnimationRepeat: " + animateType);
            }
        });
...
//                        bgView.getLayoutParams().height = fgView.getHeight();
                ...
                if (swipeable && bgVisible && touchedPosition != bgVisiblePosition) {
                    handler.removeCallbacks(mLongPressed);
//                    Log.i("connie", "touchedPosition: " + touchedPosition + " is different from bgVisiblePosition: " +
//				                    bgVisiblePosition + " closing visible");
                    // we need to close the foreground view corresponding to the bgVisibleView
                    closeVisibleBG(mOnSwipeListener, findRootFrameLayout(bgVisibleView));
                }
                break;
            }
...
//                float absVelocityY = Math.abs(mVelocityTracker.getYVelocity());
                ...
                // it's original position (spring effect)
                    ...
                    animateFG(touchedView, Animation.CLOSE, ANIMATION_STANDARD, new OnSwipeListener() {
                        @Override
                        public void onSwipeOptionsClosed(final View foregroundView, final View backgroundView) {
                            if (tempBgView != null)
                                tempBgView.setVisibility(View.VISIBLE);
//                            if (mOnSwipeListener != null) {
//                                mOnSwipeListener.onSwipeOptionsClosed(foregroundView, backgroundView);
//                            }
                        }

                        @Override
                        public void onSwipeOptionsOpened(final View foregroundView, final View backgroundView) {
//                            if (mOnSwipeListener != null) {
//                                mOnSwipeListener.onSwipeOptionsOpened(foregroundView, backgroundView);
//                            }
                        }

                        @Override
                        public void onForegroundAnimationStart(boolean isOpening, final long duration, View foregroundView, @Nullable View
                                backgroundView) {
//                            if (mOnSwipeListener != null) {
//                                mOnSwipeListener.onForegroundAnimationStart(true, duration, foregroundView, backgroundView);
//                            }
                        }
                    });
...
//                        }
    ...
    public void getTouchCoordinates(MotionEvent ev) {
        ...
        if (swipeable && bgVisible && ev.getActionMasked() == MotionEvent.ACTION_DOWN
                && y < heightOutsideRView) closeVisibleBG(mOnSwipeListener, findRootFrameLayout(bgVisibleView));
    }
    ...
    public interface OnSwipeListener {
        void onSwipeOptionsClosed(View foregroundView, @Nullable View backgroundView);

        void onSwipeOptionsOpened(View foregroundView, @Nullable View backgroundView);

        void onForegroundAnimationStart(boolean isFgOpening, long duration, View foregroundView, @Nullable View backgroundView);
    }
}
